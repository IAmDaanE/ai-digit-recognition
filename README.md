...
The neural network for my models has 3 hidden layers with a size 128 nodes, uses relu activation for the hidden layers and softmax for the output layer, which makes all outputs positive and boosts the big confident ones. The loss function is cross entropy loss which is usually implemented with softmax. With this setup I managed to get a 99.5% accuracy.
